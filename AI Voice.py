...
def greet():
    hour = int(datetime.datetime.now().hour) #creates a datetime object containing the current local time
    ...
    speak(f"its {strtime}", ) #f(formatted string literals that contain replacement fields in curley brackets), used to
    #convert date and time objects to their string representation
...
def screenshot():
    ...
    img.save("C:\\bee screenshots\\be.png") #passing name of screenshot to the save() function
...
if __name__ == '__main__': #main function
    greet()
    while True:
        query = takecommand().lower() # all the commands said by user will be stored here in 'query' and will be
        ...
        if "time" in query:
            ...
            #calling function
             ...
             search = "temprature in phagwara" #statement storing in the search variable
             url = f"https://www.google.com/search?q={search}" #this will search for the result that is stored in search
             r = requests.get(url)
             data = BeautifulSoup(r.text, "html.parser") #helps in filter out the text from the result
             temp = data.find("div", class_="BNeawe").text #here it is stored text into temperature
             speak(f"current{search} is {temp}") #Calling function
        elif "date" in query:
              date()
           ...
           try:
            ...
            webbrowser.open_new_tab("https://ums.lpu.in/lpuums/")
              ...
              music = 'C:\\Music' #location of the music
              ...
              if "yes" in reply:
                  os.system('shutdown /s /t 1') #method execute the command in a subshell, it code helps in system shutdown
                   ...
                   break
        elif "write down notes" in query:
              speak("what to notedown")
              note = takecommand()
              remember = open("data.txt", 'w') #a statement given to open a text document and giving write permissions in remember
              remember.write(note) #recording the notes
              remember.close()
              speak("Notes complete" + note)
        elif "do you have notes" in query:
